...
def AdicionarFilme():
	...
	INSERT("filme",[f.GetCpf(),str(IDFILME),nome,sinopse,strduracaominuto,strduracaohora])
	...
	for i in range(linhas):
		...
		INSERT("sessao",[str(dia[i]),str(mes[i]),str(ano[i]),str(IDSESSAO),str(numsala[i]),str(IDFILME),str(iniciominuto[i]),str(iniciohora[i])])
